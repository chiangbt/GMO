...
export default {
    ...
    methods: {
        doDeleteRole(r) {
            ...
            .then(() => {
                this.deleteRequest("/api/system/perm/role/" + r.id).then(
                    (resp) => {
                        if (resp) {
                            this.initRoles();
                        }
        ...
        doAddRole() {
            ...
            if (this.role.name && this.role.namezh) {
                this.postRequest("/api/system/perm/role", this.role).then(
                    (resp) => {
                        if (resp) {
                            this.initRoles();
                            this.role.name = "";
        ...
        doUpdate(rid, index) {
            ...
            let url = "/api/system/perm/menuid/" + rid;
            var mids = [];
        ...
        initselectedMenus(rid) {
            this.getRequest("/api/system/perm/menuid/" + rid).then((resp) => {
                if (resp) {
        ...
        initAllMenus() {
            this.getRequest("/api/system/perm/menus").then((resp) => {
                if (resp) {
                    this.allMenus = resp;
                }
            });
        ...
        initRoles() {
            this.getRequest("/api/system/perm/").then((resp) => {
                if (resp) {
                    this.roles = resp;
                }
            });
